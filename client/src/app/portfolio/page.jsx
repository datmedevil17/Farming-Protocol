...
import { mapContractDataToDeck, mapContractDataToInvestment, fetchPlatformTokenDetails, formatBalance } from '@/lib/utils';
  ...
  return (
    <div className="space-y-8">
      ...
      <Card padding='p-0' className='!bg-gray-850 overflow-hidden'>
         ...
         {error && <p className="p-6 text-center text-red-400 text-sm">Error loading investments: {error.shortMessage || error.message}</p>}
         {!isLoading && !error && userInvestments.length > 0 && ( <div>{userInvestments.map((investment, index) => ( investment && <PortfolioRow key={`${investment.deckId}-${index}`} investment={investment} onActionSuccess={handleRefetch} /> ))}</div> )}
         {!isLoading && !error && userInvestments.length === 0 && ( <p className="p-10 text-center text-gray-500">You have no active investments yet.</p> )}
      </Card>
    </div> );
}