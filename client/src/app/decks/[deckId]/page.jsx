...
import { useAccount, useReadContract } from 'wagmi'; // <<< Added useAccount
...
import AddProfitForm from '@/components/AddProfitForm'; // <<< Import AddProfitForm
...
export default function DeckDetailPage() {
  ...
  const { address, isConnected, chain } = useAccount(); // <<< Get connected address
  ...
  // Fetch deck details
  const { data: deck, isLoading: isLoadingDeck, error, refetch } = useReadContract({ // <<< Keep refetch here
  ...
  // Determine if the connected user is the creator
    ...
    <div className="space-y-8 sm:space-y-10">
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10 items-start">
          ...
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
             ...
             <DeckStats deck={deck} />
             {/* <<< Conditionally render AddProfitForm for creator >>> */}
             {isCreator && tokenDetails && (
                <AddProfitForm
                    deck={deck}
                    tokenDetails={tokenDetails}
                    onProfitAdded={refetch} // Pass refetch to update DeckStats after adding profit
                />
             )}
          </div>
