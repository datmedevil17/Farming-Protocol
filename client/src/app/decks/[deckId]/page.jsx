...
const InfoRow = ({ label, value, valueClassName = "", children }) => (
  <div>
    ...
    {children || (
      <span className={`font-semibold text-gray-100 ${valueClassName}`}>
        {value || "N/A"}
      </span>
    )}
  </div>
);
const DeckDetailInfo = ({ deck, chain }) => {
  const explorerUrl = chain?.blockExplorers?.default?.url;
  const creatorLink =
    explorerUrl && deck.creator
      ? `${explorerUrl}/address/${deck.creator}`
      : "#";
  const tokenLink =
    explorerUrl && deck.token ? `${explorerUrl}/address/${deck.token}` : "#";
  return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-xs sm:text-sm">
        <InfoRow
          ...
          valueClassName={`capitalize font-semibold ${
            deck.status === "active" ? "text-green-400" : "text-red-400"
          }`}
        />
        {/* <InfoRow
          label="Risk Level"
          value={deck.riskLevel}
          valueClassName={
            deck.riskLevel === "Low"
              ? "text-green-400"
              : deck.riskLevel === "Medium"
              ? "text-yellow-400"
              : "text-red-400"
          }
        /> */}
        <InfoRow label="Investment Asset">
          <Link
            href={tokenLink}
            target="_blank"
            rel="noopener noreferrer"
            className="font-semibold text-purple-400 hover:text-purple-300 hover:underline underline-offset-2 decoration-dotted"
            title={deck.token}
          >
            {deck.tokenSymbol || "N/A"}
          </Link>
        </InfoRow>
        {deck.minInvestment !== undefined && deck.minInvestment > 0n && (
          <InfoRow
        ...
        <InfoRow label="Creator">
          <Link
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="font-mono text-purple-400 hover:text-purple-300 hover:underline underline-offset-2 decoration-dotted"
            title={deck.creator}
          >
            {shortenAddress(deck.creator)}
          </Link>
        </InfoRow>
        {deck.strategy && <InfoRow label="Strategy" value={deck.strategy} />}
      </div>
    </Card>
  );
};
const DeckStats = ({ deck }) => {
  if (!deck) {
    return (
      <Card className="!bg-gray-800/80">
        <p className="text-sm text-gray-500">Statistics unavailable.</p>
      </Card>
    );
  }
  const formattedTVL = formatBalance(
    deck.totalInvestment,
    deck.tokenDecimals,
    0
  );
  const formattedProfit = formatBalance(
    deck.profitGenerated,
    deck.tokenDecimals,
    2
  );
  return (
    <Card className="!bg-gray-800/80">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-xs sm:text-sm">
        <InfoRow
          label="Total Invested (TVL)"
          value={`${formattedTVL} ${deck.tokenSymbol}`}
        />
        <InfoRow
          label="Total Profit Generated"
          value={`${formattedProfit} ${deck.tokenSymbol}`}
        />
        <InfoRow
          label="Est. APY"
          value={
            deck.currentAPY !== undefined
              ? `${deck.currentAPY.toFixed(1)}%`
              : "-"
          }
        />
        <InfoRow
          label="Investor Count"
          value={deck.investorsCount?.toString()}
        />
      </div>
    </Card>
  );
...
export default function DeckDetailPage() {
  ...
  const {
    ...
    query: {
      enabled:
        deckIdNumeric !== undefined &&
        !isNaN(deckIdNumeric) &&
        !!chain?.id &&
        !!tokenDetails &&
        !!investmentDeckManagerAddress,
      select: (data) =>
        mapContractDataToDeck(data, deckIdNumeric, tokenDetails),
    },
  });
  ...
  const isCreator =
    isConnected &&
    deck &&
    address?.toLowerCase() === deck.creator?.toLowerCase();
  ...
  const handleFetchAnalysis = async () => {
    ...
    try {
      ...
      const formattedTotalInvestment = formatBalance(
        deck.totalInvestment,
        tokenDecimals,
        2
      );
      const formattedProfitGenerated = formatBalance(
        deck.profitGenerated,
        tokenDecimals,
        2
      );
      const formattedMinInvestment = formatBalance(
        deck.minInvestment,
        tokenDecimals,
        2
      );
      ...
      // --- Create Prompt ---
      const prompt = `
            Analyze the following current snapshot data for investment Deck ID ${
              deck.id
            }:
            ...
            Total Profit Added by Creator/Owner: ${
              currentDataForPrompt.profitGenerated
            }
      ...
      if (!rawText) {
        throw new Error("No analysis text provided by AI model.");
      }
      ...
      try {
        ...
        if (!jsonString) {
          throw new Error(
            "Could not extract potential JSON from the response."
          );
        }

        analysisResult = JSON.parse(jsonString);
        console.log(
          `Frontend: Successfully parsed Gemini JSON for deck ID: ${deck.id}`
        );

        // Basic validation
        if (!analysisResult.summary || !analysisResult.insights) {
          throw new Error("Parsed JSON missing expected keys.");
        }
        ...
        // <<< Use a different variable name like parseErr
        console.error(
          `Frontend: Failed parse Gemini JSON for deck ${deck.id}:`,
          parseErr
        ...
        setAnalysisData({
          // Set fallback data including rawText
  ...
  if (deckError || (!isLoadingPage && !deck)) {
    const message = !deck
      ? `Could not find details for deck ID "${deckIdParam}".`
      : `Error loading deck: ${deckError?.shortMessage || deckError?.message}`;
    return (
    ...
    <>
      ...
      <div className="space-y-8 sm:space-y-10">
        <div className="flex items-center justify-between relative">
          <Button onClick={() => router.back()} variant="ghost" small>
            {" "}
            ← Back to Decks{" "}
          </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10 items-start">
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            ...
            {isCreator && tokenDetails && deck && (
              <AddProfitForm
                deck={deck}
                tokenDetails={tokenDetails}
                onProfitAdded={refetch}
              />
            )}
          </div>
        ...
        {isAnalysisModalOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-gray-900 border border-indigo-700/50 rounded-lg shadow-xl max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col"
            ...
            >
              <div className="flex justify-between items-center p-4 border-b border-gray-700/50 flex-shrink-0">
                <h3 className="text-lg font-semibold text-indigo-300 flex items-center">
                  {" "}
                  <BrainCircuit className="w-5 h-5 mr-2" /> AI Deck Analysis
                  (Snapshot)
                </h3>
                <button
                  onClick={() => setIsAnalysisModalOpen(false)}
                  className="p-1 rounded-full text-gray-500 hover:text-gray-200 hover:bg-gray-700 transition-colors"
                  title="Close"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="p-5 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                ...
                {analysisData && !isLoadingAnalysis && !analysisError && (
                  <div className="space-y-3 text-sm text-gray-300">
                    <div>
                      <p className="font-medium text-gray-100 mb-1">Summary:</p>
                    ...
                    <div>
                      ...
                      <p className="text-gray-400">
                        {analysisData.recommendation || "N/A"}
                      </p>
                    </div>
                    {analysisData.rawText && (
                      <div className="mt-4 pt-3 border-t border-gray-700">
                        <p className="font-medium text-yellow-400 text-xs mb-1">
                          Raw AI Output:
                        </p>
                        <pre className="text-xs whitespace-pre-wrap bg-gray-800 p-2 rounded font-mono max-h-24 overflow-y-auto">
                          {analysisData.rawText}
                        </pre>
                      </div>
                    )}
              ...
              <div className="p-4 border-t border-gray-700/50 text-right flex-shrink-0">
                <Button
                  variant="secondary"
                  onClick={() => setIsAnalysisModalOpen(false)}
                >
